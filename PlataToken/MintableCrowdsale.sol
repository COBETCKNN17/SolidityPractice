 ...
 */
 ...
 contract MintedCrowdsale is SafeMath, PlataToken{
    ...
    String public PlataToken;
    // Wallet address where funds are collected.
    address public collectedAddress;
    ...
    */
    function MintedCrowdsale (uint _tokensPerWei, address _collectedAddress, _myAddress) {
    // Set conditions with require statements to make sure the rate is a positive number and the addresses are non-zero.
        require(tokensPerWei > 0, "Tokens per wei must be positive number");
        require(collectedAddress.balance > 0, "collected address cannot be 0");
        require(myAddress > 0, "Source Wallet has no tokens.");

    // Set inputs as defined state variables
        tokensPerWei = _tokensPerWei;
        collectedAddress = _collectedAddress;
    ...
    function _preValidatePurchase(address senderAddress, uint purchaseAmount) {
